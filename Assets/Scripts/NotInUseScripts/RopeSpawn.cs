...
public class RopeSpawn : MonoBehaviour
{
    ...
    public void Spawn()
    {
        ...
        for (int x=0; x<count; x++)
        {
            ...
            //Vector3 pos = new Vector3(transform.position.x, transform.position.y + partDistance * (x + 1), transform.position.z);
            Vector3 pos = new Vector3(parentObject.transform.position.x, parentObject.transform.position.y + partDistance * (x + 1), parentObject.transform.position.z);
            tmp = Instantiate(partPrefab, pos, Quaternion.identity, parentObject.transform);
